...
type PerEmployeeResult = {
  services: {
    serviceId: string; serviceDate: string; customerName: string
    totalPrice: number; tipAmount: number; sharePct: number
    basePay: number; deductionPct: number; deduction: number
...
function PeriodReview({
  ...
  // Local tip state: keyed by serviceId. Starts null (use server value), updates on input.
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Seed tip inputs from server values
  ...
  const grandTips = rows.reduce((sum, r) => {
    const tip = parseFloat(tipInputs[r.serviceId] ?? '') || r.tipAmount || 0
    return sum + tip
  }, 0)
                    ...
                    <div className="space-y-0.5">
                      {row.assignments.map((a) => (
                        <div key={a.userId} className="text-xs">
                          ...
                          <span className="text-muted-foreground ml-1">({a.sharePct}%)</span>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tfoot>
                    <tr className="border-t bg-muted/40 font-semibold">
                      ...
                      <td className="px-3 py-2 text-right tabular-nums text-red-600">-{fmt(payData.summary.totalDeductions)}</td>
